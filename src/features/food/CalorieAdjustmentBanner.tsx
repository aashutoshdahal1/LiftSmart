          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center justify-between gap-2 flex-wrap">
              <p className={`text-xs font-bold uppercase tracking-wide ${c.text}`}>
                {adj.direction === "on-track" ? "On track" :
                 adj.direction === "increase"  ? `+${Math.abs(adj.delta)} kcal today` :
                                                 `-${Math.abs(adj.delta)} kcal today`}
              </p>
              {adj.delta !== 0 && (
                <p className="text-xs text-muted-foreground tabular-nums">
                  → {adj.adjustedTarget.toLocaleString()} kcal target
                </p>
              )}
            </div>
